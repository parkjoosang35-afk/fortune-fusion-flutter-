// 상점 카탈로그 조회 API — bokjumeoni-plan §03 SERVER API `GET /shop/talismans` 대응.
//
// itemType='talisman'인 활성(isActive=true) ShopCatalogItem만 표시우선순위
...
export async function GET() {
  try {
    const items = await prisma.shopCatalogItem.findMany({
      where: { itemType: "talisman", isActive: true, deletedAt: null },
    ...
    console.error("[GET /api/public/shop/talismans] 실패:", e);
    return NextResponse.json(
