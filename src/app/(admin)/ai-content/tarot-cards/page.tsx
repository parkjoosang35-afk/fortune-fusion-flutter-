...
export default async function TarotCardsPage() {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-white">타로카드 마스터 관리</h1>
      ...
      <section className="mt-6 rounded-xl border border-dashed border-slate-700 bg-slate-900/40 p-5">
        <h2 className="mb-3 text-sm font-semibold text-slate-300">
          등록된 타로 스프레드 (참조용)
        </h2>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-4">
          {spreads.map((s) => (
            <div key={s.id} className="rounded-lg border border-slate-800 bg-slate-800/40 p-3">
