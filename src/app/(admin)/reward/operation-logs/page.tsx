...
export default async function RewardOperationLogsPage({
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-white">운영로그 / 내역확인</h1>
        <p className="mt-1 text-sm text-slate-400">
          복주머니 적립/사용 내역과 최근 발급된 프리패스를 조회합니다(조회 전용).
          관리자 계정의 CRUD 감사로그는{" "}
          <Link href="/audit-logs" className="text-indigo-400 hover:underline">
            운영/보안 &gt; 감사로그
          </Link>
          에서 확인할 수 있습니다.
        </p>
      ...
      <section className="mb-8">
        <h2 className="mb-3 text-lg font-semibold text-white">복주머니 적립/사용 내역</h2>
        <form
          method="GET"
        ...
        >
          ...
          <input
            ...
            className="w-72 rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-white outline-none placeholder:text-slate-500 focus:border-indigo-500"
          />
          <button
            type="submit"
            ...
            <tbody>
              ...
              {histories.map((h) => (
                <tr key={h.id} className="border-b border-slate-800/60 hover:bg-slate-800/40">
                  ...
                  <td className="px-4 py-3 text-slate-300">{h.balanceAfter.toLocaleString()}개</td>
      ...
      {/* 최근 발급된 프리패스 */}
      <section>
        <h2 className="mb-3 text-lg font-semibold text-white">최근 발급된 프리패스</h2>
        <div className="overflow-x-auto rounded-xl border border-slate-800 bg-slate-900">
          <table className="w-full text-left text-sm">
            <thead className="border-b border-slate-800 text-xs uppercase text-slate-500">
              <tr>
            ...
            <tbody>
              ...
              {recentPasses.map((p) => (
                <tr key={p.id} className="border-b border-slate-800/60 hover:bg-slate-800/40">
                  ...
                  <td className="px-4 py-3 text-slate-200">{p.user.nickname}</td>
                  <td className="px-4 py-3 text-slate-200">{p.policy.name}</td>
                  <td className="px-4 py-3 font-mono text-slate-400">{p.sourceType}</td>
                  <td className="px-4 py-3 text-slate-400">
