...
export default async function OpenPassAttachmentsPage() {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-white">리워드 관리 — 열림패스 첨부파일</h1>
      ...
      <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div className="rounded-xl border border-slate-800 bg-slate-900 p-4">
          <p className="text-sm text-slate-400">전체 첨부파일 수</p>
          <p className="mt-1 text-2xl font-bold text-white">{attachments.length}</p>
        </div>
        <div className="rounded-xl border border-slate-800 bg-slate-900 p-4">
          <p className="text-sm text-slate-400">활성 첨부파일 수</p>
          <p className="mt-1 text-2xl font-bold text-emerald-400">{activeCount}</p>
        </div>
        <div className="rounded-xl border border-slate-800 bg-slate-900 p-4">
          <p className="text-sm text-slate-400">사용 중인 파일 유형 수</p>
          <p className="mt-1 text-2xl font-bold text-amber-400">{typeCount}</p>
        </div>
      </div>
