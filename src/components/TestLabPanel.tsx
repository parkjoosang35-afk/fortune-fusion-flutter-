  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-4">
        <h2 className="mb-3 text-sm font-semibold text-slate-900">1. 테스트 대상 유저 선택</h2>
        <div className="flex flex-wrap items-center gap-2">
          <input
            type="number"
            value={userId}
            onChange={(e) => setUserId(Number(e.target.value))}
            className="w-24 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus:border-indigo-500"
            min={1}
          />
          <button
          ...
          <input
            ...
            className="w-48 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus:border-indigo-500"
          />
          <button
            onClick={() =>
              startTransition(async () => {
        ...
        {snapshot && (
          <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
            ...
            <div className="rounded-lg border border-slate-200 bg-white p-3">
              ...
              <p className={`text-sm font-medium ${snapshot.isOpenPassActive ? "text-emerald-700" : "text-slate-500"}`}>
      ...
      {/* 2) 프리패스 테스트 */}
      <section className="rounded-xl border border-slate-200 bg-white p-4">
        <h2 className="mb-3 text-sm font-semibold text-slate-900">2. 프리패스 테스트</h2>
        ...
        <div className="flex flex-wrap gap-2">
          <button disabled={isPending || !selectedPolicyId} onClick={() => run(() => adminGrantOpenPass({ userId, policyId: selectedPolicyId!, durationOverrideMin: 30 }))} className="rounded-lg border border-slate-300 px-3 py-1.5 text-xs text-slate-600 hover:bg-slate-100 disabled:opacity-50">30분 지급</button>
          <button disabled={isPending || !selectedPolicyId} onClick={() => run(() => adminGrantOpenPass({ userId, policyId: selectedPolicyId!, durationOverrideMin: 60 }))} className="rounded-lg border border-slate-300 px-3 py-1.5 text-xs text-slate-600 hover:bg-slate-100 disabled:opacity-50">1시간 지급</button>
          <button disabled={isPending || !selectedPolicyId} onClick={() => run(() => adminGrantOpenPass({ userId, policyId: selectedPolicyId!, durationOverrideMin: 1440 }))} className="rounded-lg border border-slate-300 px-3 py-1.5 text-xs text-slate-600 hover:bg-slate-100 disabled:opacity-50">24시간 지급</button>
          <button disabled={isPending || !selectedPolicyId} onClick={() => run(() => adminGrantOpenPass({ userId, policyId: selectedPolicyId!, scopeOverride: "fortune_today" }))} className="rounded-lg border border-slate-300 px-3 py-1.5 text-xs text-slate-600 hover:bg-slate-100 disabled:opacity-50">fortune_today만 지급</button>
          <button disabled={isPending || !snapshot?.activeOpenPass} onClick={() => run(() => adminForceExpireOpenPass(snapshot!.activeOpenPass!.id))} className="rounded-lg border border-red-300 px-3 py-1.5 text-xs text-red-700 hover:bg-red-100 disabled:opacity-50">강제 만료</button>
          <button disabled={isPending || !snapshot?.activeOpenPass} onClick={() => run(() => adminRevokeOpenPass(snapshot!.activeOpenPass!.id))} className="rounded-lg border border-red-300 px-3 py-1.5 text-xs text-red-700 hover:bg-red-100 disabled:opacity-50">회수(revoke)</button>
          <button disabled={isPending || !selectedPolicyId} onClick={() => run(() => adminSimulateOpenPassSource({ userId, policyId: selectedPolicyId!, source: "ad_reward" }))} className="rounded-lg bg-indigo-100 px-3 py-1.5 text-xs text-indigo-800 hover:bg-indigo-100 disabled:opacity-50">광고보상 지급 시뮬</button>
          <button disabled={isPending || !selectedPolicyId} onClick={() => run(() => adminSimulateOpenPassSource({ userId, policyId: selectedPolicyId!, source: "purchase" }))} className="rounded-lg bg-indigo-100 px-3 py-1.5 text-xs text-indigo-800 hover:bg-indigo-100 disabled:opacity-50">구매 지급 시뮬</button>
          <button disabled={isPending || !selectedPolicyId} onClick={() => run(() => adminSimulateOpenPassSource({ userId, policyId: selectedPolicyId!, source: "event" }))} className="rounded-lg bg-indigo-100 px-3 py-1.5 text-xs text-indigo-800 hover:bg-indigo-100 disabled:opacity-50">이벤트 지급 시뮬</button>
          <button disabled={isPending || !selectedPolicyId} onClick={() => run(() => adminSimulateOpenPassSource({ userId, policyId: selectedPolicyId!, source: "test_mode" }))} className="rounded-lg bg-indigo-100 px-3 py-1.5 text-xs text-indigo-800 hover:bg-indigo-100 disabled:opacity-50">테스트 지급 시뮬</button>
        </div>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-slate-200 pt-3">
          ...
          <input
            ...
            min={1}
            className="w-20 rounded-lg border border-slate-300 bg-white px-2 py-1.5 text-xs text-slate-900 outline-none focus:border-indigo-500"
          />
          <button
          ...
          <button
            ...
            className="rounded-lg border border-sky-300 px-3 py-1.5 text-xs text-sky-700 hover:bg-sky-100 disabled:opacity-50"
          >
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-4">
        ...
        <div className="mb-3 flex flex-wrap items-center gap-2">
          ...
          <button disabled={isPending} onClick={() => run(() => adminManualGrantLuckPouch({ userId, amount: lpAmount, memo: lpMemo }))} className="rounded-lg border border-slate-300 px-3 py-1.5 text-xs text-slate-600 hover:bg-slate-100 disabled:opacity-50">수동 지급</button>
          <button disabled={isPending} onClick={() => run(() => adminManualDeductLuckPouch({ userId, amount: lpAmount, memo: lpMemo }))} className="rounded-lg border border-red-300 px-3 py-1.5 text-xs text-red-700 hover:bg-red-100 disabled:opacity-50">수동 차감</button>
        </div>
        <div className="mb-3 flex flex-wrap items-center gap-2">
          <span className="text-xs text-slate-500">적립 액션:</span>
          <select value={selectedEarnRuleId} onChange={(e) => setSelectedEarnRuleId(Number(e.target.value))} className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus:border-indigo-500">
            {earnRules.map((r) => (<option key={r.id} value={r.id}>{r.name} (+{r.amount})</option>))}
          </select>
          <button disabled={isPending || !selectedEarnRuleId} onClick={() => run(() => adminSimulateLuckPouchRule({ userId, ruleId: selectedEarnRuleId! }))} className="rounded-lg bg-indigo-100 px-3 py-1.5 text-xs text-indigo-800 hover:bg-indigo-100 disabled:opacity-50">적립 실행</button>
        </div>
        <div className="mb-3 flex flex-wrap items-center gap-2">
          <span className="text-xs text-slate-500">소비 액션(응원/공감/강조/노출강화/부적):</span>
          <select value={selectedSpendRuleId} onChange={(e) => setSelectedSpendRuleId(Number(e.target.value))} className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus:border-indigo-500">
            {spendRules.map((r) => (<option key={r.id} value={r.id}>{r.name} (-{r.amount})</option>))}
          </select>
          <button disabled={isPending || !selectedSpendRuleId} onClick={() => run(() => adminSimulateLuckPouchRule({ userId, ruleId: selectedSpendRuleId! }))} className="rounded-lg bg-indigo-100 px-3 py-1.5 text-xs text-indigo-800 hover:bg-indigo-100 disabled:opacity-50">사용 실행</button>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <span className="text-xs text-slate-500">구매:</span>
          <select value={selectedPurchaseRuleId} onChange={(e) => setSelectedPurchaseRuleId(Number(e.target.value))} className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus:border-indigo-500">
            {purchaseRules.map((r) => (<option key={r.id} value={r.id}>{r.name}</option>))}
          </select>
          <button disabled={isPending || !selectedPurchaseRuleId} onClick={() => run(() => adminSimulateLuckPouchRule({ userId, ruleId: selectedPurchaseRuleId! }))} className="rounded-lg bg-indigo-100 px-3 py-1.5 text-xs text-indigo-800 hover:bg-indigo-100 disabled:opacity-50">구매 실행 시뮬</button>
        </div>
      </section>
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-4">
        ...
        <div className="mb-3 flex flex-wrap items-center gap-2">
          ...
          <select value={selectedAdSourceId} onChange={(e) => setSelectedAdSourceId(Number(e.target.value))} className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus:border-indigo-500">
            {adSources.map((a) => (<option key={a.id} value={a.id}>{a.sourceName}{a.isActive ? "" : " (비활성)"}</option>))}
          </select>
        </div>
        <div className="flex flex-wrap gap-2">
          <button disabled={isPending || !selectedPolicyId || !selectedAdSourceId} onClick={() => run(() => adminSimulateAdRewardSuccess({ userId, policyId: selectedPolicyId!, adSourceId: selectedAdSourceId! }))} className="rounded-lg bg-emerald-100 px-3 py-1.5 text-xs text-emerald-700 hover:bg-emerald-100 disabled:opacity-50">광고 성공→보상지급</button>
          <button disabled={isPending || !selectedAdSourceId} onClick={() => run(() => adminSimulateAdRewardFail({ userId, adSourceId: selectedAdSourceId!, policyId: selectedPolicyId }))} className="rounded-lg border border-red-300 px-3 py-1.5 text-xs text-red-700 hover:bg-red-100 disabled:opacity-50">광고 실패→fallback</button>
          <button disabled={isPending || !selectedAdSourceId} onClick={() => run(() => adminSimulateAdRewardNoFill({ userId, adSourceId: selectedAdSourceId!, policyId: selectedPolicyId }))} className="rounded-lg border border-amber-300 px-3 py-1.5 text-xs text-amber-700 hover:bg-amber-100 disabled:opacity-50">no-fill→fallback</button>
          <button disabled={isPending || !selectedAdSourceId} onClick={() => run(() => adminSimulateAdRewardCancel({ userId, adSourceId: selectedAdSourceId!, policyId: selectedPolicyId }))} className="rounded-lg border border-orange-300 px-3 py-1.5 text-xs text-orange-700 hover:bg-orange-100 disabled:opacity-50">중도취소→fallback</button>
          <button disabled={isPending || !selectedAdSourceId} onClick={() => run(() => adminSimulateAdRewardTimeout({ userId, adSourceId: selectedAdSourceId!, policyId: selectedPolicyId }))} className="rounded-lg border border-rose-300 px-3 py-1.5 text-xs text-rose-700 hover:bg-rose-100 disabled:opacity-50">타임아웃→fallback</button>
          <button disabled={isPending || !selectedPolicyId} onClick={() => run(() => adminPreviewProductAdConfig({ policyId: selectedPolicyId! }), false)} className="rounded-lg border border-slate-300 px-3 py-1.5 text-xs text-slate-600 hover:bg-slate-100 disabled:opacity-50">상품 광고설정 미리보기</button>
        </div>
      </section>

      {/* 6) 최근 시뮬레이션 결과 */}
      <section className="rounded-xl border border-slate-200 bg-white p-4">
        <h2 className="mb-3 text-sm font-semibold text-slate-900">6. 최근 시뮬레이션 결과</h2>
        {logs.length === 0 ? (
            ...
            {logs.map((log) => (
              <li
                ...
                className={`whitespace-pre-wrap rounded-lg border px-3 py-2 text-xs ${
                  log.ok ? "border-emerald-300/60 bg-emerald-100 text-emerald-800" : "border-red-300/60 bg-red-100 text-red-800"
                }`}
              >
