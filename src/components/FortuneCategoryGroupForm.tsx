    ...
    <div className="rounded-xl border border-slate-200 bg-white p-5">
      <div className="mb-3 flex items-center justify-between">
        <span className="text-xs text-slate-500">
          code: <span className="font-mono text-slate-500">{code}</span> · 카테고리{" "}
          {categoryCount}개
        </span>
      ...
      <form action={formAction} className="space-y-3">
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-[2fr_3fr_1fr]">
          <div>
            <label className="mb-1 block text-xs font-medium text-slate-500">그룹명</label>
            <input
              name="label"
              defaultValue={label}
              disabled={!canWrite}
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-slate-500">정렬순서</label>
            <input
              type="number"
              name="displayOrder"
        ...
        {state.success && <p className="text-xs text-emerald-700">저장되었습니다.</p>}
