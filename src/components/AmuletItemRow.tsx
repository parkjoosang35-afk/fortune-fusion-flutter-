  ...
  return (
    <tr className="border-b border-slate-800/60 hover:bg-slate-800/40">
      ...
      <td className="px-4 py-3 text-slate-200">
        {item.name}
        {item.isLimited && (
          <span className="ml-2 rounded-full bg-amber-950/60 px-2 py-0.5 text-xs text-amber-400">
            한정판
          </span>
        )}
      </td>
      <td className="px-4 py-3 text-slate-300">{gradeName}</td>
      <td className="px-4 py-3 max-w-[240px] truncate text-slate-400" title={item.effectDescription}>
        {item.effectDescription}
      </td>
      <td className="px-4 py-3 text-slate-300">{item.pricePoint.toLocaleString()}P</td>
      <td className="px-4 py-3">
