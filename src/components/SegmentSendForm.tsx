...
export default function SegmentSendForm({
  ...
  return (
    ...
    >
      <h3 className="col-span-full text-sm font-semibold text-slate-900">세그먼트 발송 실행</h3>
      ...
      {targetType === "condition" && (
        <>
          <fieldset className="col-span-full grid grid-cols-1 gap-3 rounded-lg border border-slate-200 p-3 md:grid-cols-3">
            <legend className="px-1 text-xs font-medium text-slate-500">조건 설정</legend>
            ...
            <div className="flex flex-col gap-1 text-xs text-slate-500">
              ...
              <div className="flex flex-wrap gap-2">
                {GRADE_OPTIONS.map((g) => (
                  <label
                    key={g.code}
                    className="flex items-center gap-1 rounded-lg border border-slate-300 bg-white px-2 py-1 text-slate-600"
                  >
                    <input
                      type="checkbox"
                      name="gradeCodes"
