...
export function buildTopicSummaryPrompt(params: {
  ...
  // 서버가 question-guard.ts로 입력 형식/의도를 이미 검증했지만, 그 규칙
  // 기반 검증이 완벽할 수 없으므로(예: 새로운 유형의 무의미한 문장) AI
  // 자신도 마지막 방어선 역할을 하도록 명시적으로 지시한다. 카드는 이미
  // 확정되어 있으므로 이 지시가 카드 해석 자체에는 영향을 주지 않는다 —
  // 오직 "사용자 질문이 상담 질문으로 부적절해 보일 때의 태도"만 규정한다.
  lines.push(
    "",
