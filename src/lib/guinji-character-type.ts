...
export const GUINJI_CHARACTER_TYPES: Record<OhaengKey, GuinjiCharacterType> = {
  ...
  수: {
    ...
    // [2026-09, 사용자 격노 피드백 반영] 원래 값(#4B7FA6, 선명한 하늘색)이
    // 아이보리+로즈골드 바이럴 팔레트 안에서 유독 튀어 "이 페이지만 리스킨이
    // 안 됐다"는 오해를 유발했다. 木土金火와 동일한 톤-다운(muted/earthy)
    // 채도로 맞추되, 水(물) 속성임을 구분할 수 있는 차분한 더스티 블루로 조정.
    color: "#6E8CA6",
  },
};
