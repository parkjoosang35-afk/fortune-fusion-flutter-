...
class JeontongReportBuilder {
  ...
  static FortuneReport? _tryBuildRealReport(
    ...
    try {
      ...
      // [j7 · A01→B01→C01→D01→A02→A03→A04→A05→A06→C02→C03 신규 엔진 순차
      // 이전 — entry.id 기준 분기] 검증 완료된 카테고리(A01, B01, C01,
      // D01, A02, A03, A04, A05, A06, C02, C03)만 PHASE1~4(만세력 단일
      // 기준 엔진) → SajuProfile → sajuResultFromProfile() 어댑터 →
