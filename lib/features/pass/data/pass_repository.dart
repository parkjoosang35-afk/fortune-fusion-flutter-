...
/// [신규] 알림패스(AlarmPass) Repository — admin_web 공개 API
...
class PassRepository {
  ...
  Future<ApiResult<List<PassPolicyModel>>> getPolicies() async {
    ...
    try {
      ...
      if (response.statusCode != 200 || decoded['success'] != true) {
        final error = decoded['error'] as String? ?? '알림패스 정책을 불러오지 못했습니다.';
      ...
      return ApiResult.fail('알림패스 정책을 불러오지 못했습니다: $e');
    }
  }

  /// GET /api/public/pass/status — 현재 알림패스 활성 상태(홈 화면 상태바)
  Future<ApiResult<PassStatusModel>> getStatus() async {
    ...
    try {
      ...
      final decoded = jsonDecode(response.body) as Map<String, dynamic>;
      if (response.statusCode != 200 || decoded['success'] != true) {
        final error = decoded['error'] as String? ?? '알림패스 상태를 불러오지 못했습니다.';
      ...
      return ApiResult.fail('알림패스 상태를 불러오지 못했습니다: $e');
    }
  }

  /// POST /api/public/pass/claim-ad — 광고 시청 완료 후 알림패스 발급
  Future<ApiResult<PassStatusModel>> claimAd({int? policyId}) async {
    return _claim('claim-ad', policyId: policyId);
  }

  /// POST /api/public/pass/claim-partner — 파트너 랜딩 방문 완료 후 알림패스 발급
  ...
  Future<ApiResult<PassStatusModel>> _claim(
    ...
    try {
      ...
      final decoded = jsonDecode(response.body) as Map<String, dynamic>;
      if (response.statusCode != 200 || decoded['success'] != true) {
        final error = decoded['error'] as String? ?? '알림패스 발급에 실패했습니다.';
      ...
      return ApiResult.fail('알림패스 발급 중 오류가 발생했습니다: $e');
    }
  }

  /// POST /api/public/pass/consume — 시간제 콘텐츠 열람 직전 게이트 체크
  /// 유효한 알림패스가 없으면 실패(ApiResult.fail)를 반환하며, 화면단에서
  /// 이 실패를 감지해 알림패스 발급 유도 UI를 노출한다.
  Future<ApiResult<void>> consume({
    ...
    try {
      ...
      final decoded = jsonDecode(response.body) as Map<String, dynamic>;
      if (response.statusCode != 200 || decoded['success'] != true) {
        final error = decoded['error'] as String? ?? '유효한 알림패스가 없습니다.';
      ...
      return ApiResult.fail('알림패스 검증 중 오류가 발생했습니다: $e');
    }
  }
}
