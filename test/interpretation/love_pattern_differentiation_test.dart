...
void main() {
  ...
  test('같은 spousePattern 그룹 내부에서 세부 근거(interpretationContext)가 서로 다르다', () {
    ...
    // ignore: avoid_print
    print(
      'spousePattern 그룹 크기: ${byPattern.map((k, v) => MapEntry(k, v.length))}',
    );
    ...
    final palaceConditions = groupUsers
        .map((u) => palaceConditionByUser[u]!)
        .toSet();
    ...
    // ignore: avoid_print
    print(
      '그룹 내부: spouseBondStrength=${bondStrengths.length}종, '
