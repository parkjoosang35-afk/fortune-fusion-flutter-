...
void main() {
  ...
  test('동일 입력은 항상 동일 Narrative(결정론)', () {
    ...
    for (var i = 0; i < 5; i++) {
      ...
      final analysis = careerAnalyzer.analyze(
        built.profile,
        referenceDate: refDate,
      );
  ...
  test('A01(계산레벨)과 A04 Narrative가 같은 사람에 대해서도 서로 다른 관점의 결과를 낸다(§9)', () {
    ...
    final a04Analysis = careerAnalyzer.analyze(
      built.profile,
      referenceDate: refDate,
    );
    ...
    expect(
      ...
      isNot(equals(a04Narrative.toJson().toString())),
    );
    ...
    for (final input in kJeontongSample120.take(30)) {
      ...
      if (analysis.careerPeakDaewoonLabel.isEmpty) {
        expect(
          ...
          isNull,
          reason:
              '${input.userId}: careerPeakDaewoonLabel이 없는데 timingSection이 생성됨(가짜 시기 위험)',
        );
      } else {
