...
void main() {
  ...
  test('같은 healthConstitutionPattern 그룹 내부에서 세부 근거(interpretationContext)가 서로 다르다', () {
    ...
    for (final input in kJeontongSample120) {
      ...
      byPattern.putIfAbsent(a.healthConstitutionPattern, () => []).add(input.userId);
    ...
    // ignore: avoid_print
    print('healthConstitutionPattern 그룹 크기: ${byPattern.map((k, v) => MapEntry(k, v.length))}');
    ...
    expect(supportingStrings.length, greaterThan(1),
        reason: '같은 healthConstitutionPattern 그룹의 supportingEvidence(§5 세부 근거)가 전원 동일하면 안 됨');
    ...
    // ignore: avoid_print
    print('그룹 내부: healthVitality=${vitalities.length}종, '
    ...
    expect(differentiatedFieldCount, greaterThanOrEqualTo(2),
        reason: '같은 healthConstitutionPattern 그룹 내부에서 최소 2개 이상의 실질 필드가 갈라져야 함(§15)');
  });
