...
void main() {
  ...
  group('§14 문장(근사치) 중복률 검사 — A06 120명', () {
    ...
    setUpAll(() {
      for (final input in kJeontongSample120) {
        ...
        final love = loveAnalyzer.analyze(built.profile, referenceDate: refDate);
        ...
        a06CoreEvidenceJudgments.add(love.coreEvidence.map((e) => e.judgment).join(' '));
      }
    });
    ...
    test('A06 spousePattern 중복률이 과도하지 않다(단, 배우자성 개수 분포 편중은 별도 findings로 기록)', () {
      // A03(재관쌍미 80%)/A04(관인상생 75%)와 마찬가지로 배우자성/비겁
      // 개수 조합 분류 특성상 특정 그룹으로 편중될 수 있어 완화된
      // 임계치(0.8)를 적용해 표본 변동에 따른 우발적 실패를 막는다.
      checkMaxDuplicateRatio('A06.spousePattern', a06SpousePattern, maxRatio: 0.8);
    });

    test('A06 spouseBondStrength 중복률이 과도하지 않다', () {
      checkMaxDuplicateRatio('A06.spouseBondStrength', a06SpouseBondStrength, maxRatio: 0.7);
    });

    test('A06 spousePalaceCondition 중복률이 과도하지 않다', () {
      checkMaxDuplicateRatio('A06.spousePalaceCondition', a06SpousePalaceCondition, maxRatio: 0.7);
    });

    test('A06 coreEvidence judgment 전체 결합문 중복률이 과도하지 않다', () {
      checkMaxDuplicateRatio('A06.coreEvidenceJudgments', a06CoreEvidenceJudgments, maxRatio: 0.5);
    });
  });
  ...
  group('§16 Evidence 추적성 검증 — A06 120명 중 대표 샘플', () {
    ...
    for (final idx in sampleIndices) {
      ...
      test('${input.userId}: A06 coreEvidence+supportingEvidence가 근거-판단 추적 요건을 만족한다', () {
        final kst = input.birthDateTimeUtc.toUtc().add(const Duration(hours: 9));
        final built = JeontongReportBuilder.buildProfileAndSajuResultViaPhase1to4(
          kst: kst,
          gender: input.gender,
        ...
        for (final key in [
          'gender', 'spouseCategory', 'spouseCount', 'biCount',
          'strengthVerdict', 'yongsinElement', 'dayBranchRelationTypes',
          'foundRomanceSinsal', 'marriagePeakDaewoonLabel',
        ]) {
