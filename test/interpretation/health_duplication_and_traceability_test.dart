...
void main() {
  ...
  group('§14 문장(근사치) 중복률 검사 — A05 120명', () {
    ...
    setUpAll(() {
      for (final input in kJeontongSample120) {
        ...
        final health = healthAnalyzer.analyze(built.profile, referenceDate: refDate);
        ...
        a05CoreEvidenceJudgments.add(health.coreEvidence.map((e) => e.judgment).join(' '));
      }
    });
    ...
    test('A05 healthConstitutionPattern 중복률이 과도하지 않다', () {
      ...
      checkMaxDuplicateRatio('A05.healthConstitutionPattern', a05HealthConstitutionPattern, maxRatio: 0.8);
    });

    test('A05 healthVitality 중복률이 과도하지 않다', () {
      checkMaxDuplicateRatio('A05.healthVitality', a05HealthVitality, maxRatio: 0.6);
    });

    test('A05 vulnerableOrgans 중복률이 과도하지 않다', () {
      checkMaxDuplicateRatio('A05.vulnerableOrgans', a05VulnerableOrgans, maxRatio: 0.7);
    });

    test('A05 coreEvidence judgment 전체 결합문 중복률이 과도하지 않다', () {
      checkMaxDuplicateRatio('A05.coreEvidenceJudgments', a05CoreEvidenceJudgments, maxRatio: 0.5);
    });
  });
  ...
  group('§16 Evidence 추적성 검증 — A05 120명 중 대표 샘플', () {
    ...
    for (final idx in sampleIndices) {
      ...
      test('${input.userId}: A05 coreEvidence+supportingEvidence가 근거-판단 추적 요건을 만족한다', () {
        final kst = input.birthDateTimeUtc.toUtc().add(const Duration(hours: 9));
        final built = JeontongReportBuilder.buildProfileAndSajuResultViaPhase1to4(
          kst: kst,
          gender: input.gender,
        ...
        for (final key in [
          'dominantElements', 'deficientElements', 'isImbalanced',
          'strengthVerdict', 'yongsinElement', 'yongsinRooted',
          'gisinElement', 'gisinCount', 'foundHealthSinsal',
          'careBaseElement', 'dayGan',
        ]) {
