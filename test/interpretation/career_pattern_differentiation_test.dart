...
void main() {
  ...
  test('같은 careerPattern 그룹 내부에서 세부 근거(interpretationContext)가 서로 다르다', () {
    ...
    // ignore: avoid_print
    print(
      'careerPattern 그룹 크기: ${byPattern.map((k, v) => MapEntry(k, v.length))}',
    );
    ...
    //    문장 이전 단계 대응 필드로 확인.
    final careerStrengths = groupUsers
        .map((u) => careerStrengthByUser[u]!)
        .toSet();
    ...
    // ignore: avoid_print
    print(
      '그룹 내부: careerStrength=${careerStrengths.length}종, '
