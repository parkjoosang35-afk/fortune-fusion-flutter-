/// [정통사주 69종 개인화 해석 엔진 — A06 독립 파이프라인 검증]
/// LoveNarrativeGenerator(문장 생성 레이어) 검증.
///
/// love_analyzer_test.dart/love_duplication_and_traceability_test.dart/
/// love_pattern_differentiation_test.dart는 LoveAnalysis(계산 레이어)를
/// 검증한다. 이 파일은 그 위에 새로 쌓인 LoveNarrativeGenerator(문장
/// 레이어)가 A01/A03/A04/A05 검증 테스트와 동일한 형식으로 결정론·개인화·
...
import 'package:flutter_app/features/home/domain/interpretation/generators/love_narrative_generator.dart';
...
void main() {
  ...
  final loveGen = const LoveNarrativeGenerator();
  ...
  test('120명의 A06 coreResult/전체 Narrative가 서로 다르다(§5 개인화)', () {
  ...
  test('A01(계산레벨)과 A06 Narrative가 같은 사람에 대해서도 서로 다른 관점의 결과를 낸다(§9)', () {
    final input = kJeontongTestInputs[0];
    final kst = input.birthDateTimeUtc.toUtc().add(const Duration(hours: 9));
    final built = JeontongReportBuilder.buildProfileAndSajuResultViaPhase1to4(
      kst: kst, gender: input.gender, isLunar: input.isLunar, referenceDate: refDate,
  ...
  test('§9 금지 문구(범용 문구)가 A06 Narrative에 포함되지 않는다(120명 전수 검사)', () {
    for (final input in kJeontongSample120) {
      final kst = input.birthDateTimeUtc.toUtc().add(const Duration(hours: 9));
      final built = JeontongReportBuilder.buildProfileAndSajuResultViaPhase1to4(
        kst: kst, gender: input.gender, isLunar: input.isLunar, referenceDate: refDate,
  ...
  test('A06 고유 근거: 배우자궁(일지) 관계가 characteristics에 항상 드러난다', () {
    for (final input in kJeontongSample120.take(30)) {
      final kst = input.birthDateTimeUtc.toUtc().add(const Duration(hours: 9));
      final built = JeontongReportBuilder.buildProfileAndSajuResultViaPhase1to4(
        kst: kst, gender: input.gender, isLunar: input.isLunar, referenceDate: refDate,
