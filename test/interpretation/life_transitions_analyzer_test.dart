/// [정통사주 69종 개인화 해석 엔진 — 10단계] A10 LifeTransitionsAnalyzer 검증.
///
/// A03~A09와 동일한 패턴 — §2(결정론) + §5/§6(개인화)를 3명 seed +
/// 30명 샘플로 확인하고, A01/A03~A10이 같은 SajuProfile을 받고도 서로
/// 다른 관점의 결과를 낸다는 §9를 함께 확인한다. A10 고유 근거인
/// turningPointsCondition(대운 앞 5개의 전환점별 상세)도 함께 검증한다.
library;
...
import 'package:flutter_app/features/home/domain/interpretation/analyzers/life_transitions_analyzer.dart';
...
void main() {
  final analyzer = const LifeTransitionsAnalyzer();
  final studyAnalyzer = const StudyAnalyzer();
  ...
  test('서로 다른 3명의 A10 결과가 실제로 달라진다', () {
    ...
    for (final input in kJeontongTestInputs) {
      ...
      print('--- ${input.userId} ---');
      // ignore: avoid_print
      print('${analysis.transitionPattern} / ${analysis.transitionBondStrength}');
      // ignore: avoid_print
      print('turningPoints=${analysis.turningPointsCondition}');
      // ignore: avoid_print
      print('risk=${analysis.transitionRiskPattern}');
      // ignore: avoid_print
      print('approach=${analysis.transitionApproach}');
      // ignore: avoid_print
      print('key=${analysis.keyTurningPointLabel}');
    }
  ...
  test('같은 SajuProfile이라도 A01/A03~A10은 서로 다른 관점의 결과를 낸다(§9)', () {
    final input = kJeontongTestInputs[0];
    final kst = input.birthDateTimeUtc.toUtc().add(const Duration(hours: 9));
    final built = JeontongReportBuilder.buildProfileAndSajuResultViaPhase1to4(
      kst: kst, gender: input.gender, isLunar: input.isLunar, referenceDate: refDate,
    ...
    final a09 = studyAnalyzer.analyze(built.profile, referenceDate: refDate);
    final a10 = analyzer.analyze(built.profile, referenceDate: refDate);
    expect(a01.categoryId, isNot(equals(a10.categoryId)));
    expect(a03.categoryId, isNot(equals(a10.categoryId)));
    expect(a04.categoryId, isNot(equals(a10.categoryId)));
    expect(a05.categoryId, isNot(equals(a10.categoryId)));
    expect(a06.categoryId, isNot(equals(a10.categoryId)));
    expect(a07.categoryId, isNot(equals(a10.categoryId)));
    expect(a08.categoryId, isNot(equals(a10.categoryId)));
    expect(a09.categoryId, isNot(equals(a10.categoryId)));
    expect(a10.transitionPattern, isNotEmpty);
    expect(a10.transitionBondStrength, isNotEmpty);
    expect(a10.turningPointsCondition, isNotEmpty);
    expect(a10.transitionRiskPattern, isNotEmpty);
    expect(a10.transitionApproach, isNotEmpty);
  });

  test('30명 샘플의 A10 transitionPattern/transitionBondStrength/turningPointsCondition이 계산값에 따라 달라진다', () {
    final transitionPatternSet = <String>{};
    final transitionBondStrengthSet = <String>{};
    final turningPointsSet = <String>{};
    ...
    for (final input in kJeontongSample30) {
      ...
      final analysis = analyzer.analyze(built.profile, referenceDate: refDate);
      transitionPatternSet.add(analysis.transitionPattern);
      transitionBondStrengthSet.add(analysis.transitionBondStrength);
      turningPointsSet.add(analysis.turningPointsCondition);
      fullJsonSet.add(analysis.toJson().toString());
    }

    expect(fullJsonSet.length, greaterThan(1));
    expect(transitionPatternSet.length, greaterThan(1),
        reason: 'transitionPattern이 30명 전원 동일하면 개인화 실패');
    expect(transitionBondStrengthSet.length, greaterThan(1),
        reason: 'transitionBondStrength가 30명 전원 동일하면 개인화 실패');
    expect(turningPointsSet.length, greaterThan(1),
        reason: 'turningPointsCondition이 30명 전원 동일하면 개인화 실패');

    // ignore: avoid_print
    print('transitionPattern 종류=$transitionPatternSet');
    // ignore: avoid_print
    print('transitionBondStrength 종류=$transitionBondStrengthSet');
  });
}
