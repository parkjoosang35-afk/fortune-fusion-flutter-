/// [정통사주 69종 개인화 해석 엔진 — 7단계] A07 ChildrenAnalyzer 검증.
///
/// A03(wealth_analyzer_test.dart)/A04(career_analyzer_test.dart)/
/// A05(health_analyzer_test.dart)/A06(love_analyzer_test.dart)와 동일한
/// 패턴 — §2(결정론) + §5/§6(개인화)를 3명 seed + 30명 샘플로 확인하고,
/// A01/A03/A04/A05/A06/A07이 같은 SajuProfile을 받고도 서로 다른 관점의
/// 결과를 낸다는 §9를 함께 확인한다.
library;
...
void main() {
  final analyzer = const ChildrenAnalyzer();
  ...
  test('서로 다른 3명의 A07 결과가 실제로 달라진다', () {
    ...
    for (final input in kJeontongTestInputs) {
      ...
      print('--- ${input.userId} ---');
      // ignore: avoid_print
      print('${analysis.childPattern} / ${analysis.childBondStrength}');
      // ignore: avoid_print
      print('palace=${analysis.childPalaceCondition}');
      // ignore: avoid_print
      print('risk=${analysis.childRiskPattern}');
      // ignore: avoid_print
      print('approach=${analysis.childRearingApproach}');
      // ignore: avoid_print
      print('peak_daewoon=${analysis.childBlessingDaewoonLabel}');
    }
  ...
  test('같은 SajuProfile이라도 A01/A03/A04/A05/A06/A07은 서로 다른 관점의 결과를 낸다(§9)', () {
    final input = kJeontongTestInputs[0];
    final kst = input.birthDateTimeUtc.toUtc().add(const Duration(hours: 9));
    final built = JeontongReportBuilder.buildProfileAndSajuResultViaPhase1to4(
      kst: kst,
    ...
    final a07 = analyzer.analyze(built.profile, referenceDate: refDate);
    expect(a01.categoryId, isNot(equals(a07.categoryId)));
    expect(a03.categoryId, isNot(equals(a07.categoryId)));
    expect(a04.categoryId, isNot(equals(a07.categoryId)));
    expect(a05.categoryId, isNot(equals(a07.categoryId)));
    expect(a06.categoryId, isNot(equals(a07.categoryId)));
    expect(a07.childPattern, isNotEmpty);
    expect(a07.childBondStrength, isNotEmpty);
    expect(a07.childPalaceCondition, isNotEmpty);
  });

  test('30명 샘플의 A07 childPattern/childPalaceCondition이 계산값에 따라 달라진다', () {
    ...
    for (final input in kJeontongSample30) {
      ...
      palaceSet.add(analysis.childPalaceCondition);
    ...
    // ignore: avoid_print
    print('childPalaceCondition 종류=$palaceSet');
  });
}
