/// [정통사주 69종 개인화 해석 엔진 — 5단계] A05 HealthAnalyzer 검증.
///
/// A03(wealth_analyzer_test.dart)/A04(career_analyzer_test.dart)와 동일한
/// 패턴 — §16(결정론) + §17(개인화)를 3명 seed + 30명 샘플로 확인하고,
/// A01/A03/A04/A05가 같은 SajuProfile을 받고도 서로 다른 관점의 결과를
/// 낸다는 §9를 함께 확인한다.
library;
...
void main() {
  final analyzer = const HealthAnalyzer();
  ...
  test('서로 다른 3명의 A05 결과가 실제로 달라진다', () {
    ...
    for (final input in kJeontongTestInputs) {
      ...
      // ignore: avoid_print
      print('risk=${analysis.healthRiskPattern}');
      // ignore: avoid_print
      print('caution_daewoon=${analysis.healthCautionDaewoonLabel}');
    }
  ...
  test('같은 SajuProfile이라도 A01/A03/A04/A05는 서로 다른 관점의 결과를 낸다(§9)', () {
    final input = kJeontongTestInputs[0];
    final kst = input.birthDateTimeUtc.toUtc().add(const Duration(hours: 9));
    final built = JeontongReportBuilder.buildProfileAndSajuResultViaPhase1to4(
      kst: kst, gender: input.gender, isLunar: input.isLunar, referenceDate: refDate,
    ...
    final a05 = analyzer.analyze(built.profile, referenceDate: refDate);
    expect(a01.categoryId, isNot(equals(a05.categoryId)));
    expect(a03.categoryId, isNot(equals(a05.categoryId)));
    expect(a04.categoryId, isNot(equals(a05.categoryId)));
    expect(a05.healthConstitutionPattern, isNotEmpty);
