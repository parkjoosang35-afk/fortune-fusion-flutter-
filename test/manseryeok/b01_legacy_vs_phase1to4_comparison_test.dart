...
void main() {
  ...
  group('[j7·B01] 레거시 vs 신규(PHASE1~4+어댑터) 전체 비교 — seed 유저 3명', () {
    for (final u in _seedUsers) {
      test('${u.userId}: 8글자/일간/오행/십신/대운/신강신약/용희기구/B01 전체 비교', () {
        ...
        // ignore: avoid_print
        print(
          '[대운] legacy(${legacyReal.length}개)=${legacyReal.map((e) => '${e.ganZhiKr}(${e.startAge}세~${e.startYear})').join(', ')}',
        );
        // ignore: avoid_print
        print(
          '[대운] new(${newB.saju.luckPillars.length}개)=${newB.saju.luckPillars.map((e) => '${e.ganZhiKr}(${e.startAge}세~${e.startYear})').join(', ')}',
        );
        for (var i = 0; i < legacyReal.length; i++) {
          ...
          expect(
            newB.saju.luckPillars[i].startAge,
            legacyReal[i].startAge,
            reason: '${u.userId} 대운[$i] 시작연령 불일치',
          );
          expect(
            newB.saju.luckPillars[i].startYear,
            legacyReal[i].startYear,
            reason: '${u.userId} 대운[$i] 시작연도 불일치',
          );
        }
        ...
        // ignore: avoid_print
        print(
          '[현재대운] legacy=${legacyB.saju.currentLuck?.ganZhiKr}(${legacyB.saju.currentLuck?.startAge}세~) currentAge=${legacyB.saju.currentAge}',
        );
        // ignore: avoid_print
        print(
          '[현재대운] new   =${newB.saju.currentLuck?.ganZhiKr}(${newB.saju.currentLuck?.startAge}세~) currentAge=${newB.saju.currentAge}',
        );
        expect(
          newB.saju.currentAge,
          legacyB.saju.currentAge,
          reason: '${u.userId} currentAge 불일치',
        );
        expect(
          newB.saju.currentLuck?.ganZhi,
          legacyB.saju.currentLuck?.ganZhi,
          reason: '${u.userId} currentLuck 간지 불일치',
        );
        ...
        // ignore: avoid_print
        print(
          '[신강신약] new   =${newB.saju.dayMasterStrength}  (${strengthSame ? "동일" : "★ 다름(A01과 동일한 seed-user-C 패턴 - B01엔 영향 없음) ★"})',
        );
        ...
        // ignore: avoid_print
        print(
          '[B01계산값·ageRange] legacy=${lLuck.ageRange}  new=${nLuck.ageRange}',
        );
        // ignore: avoid_print
        print(
          '[B01계산값·ganGod] legacy=${lLuck.ganGodName}(${lLuck.ganGodEasy})  new=${nLuck.ganGodName}(${nLuck.ganGodEasy})',
        );
        // ignore: avoid_print
        print(
          '[B01계산값·zhiGod] legacy=${lLuck.zhiGodName}(${lLuck.zhiGodEasy})  new=${nLuck.zhiGodName}(${nLuck.zhiGodEasy})',
        );
        // ignore: avoid_print
        ...
        // ignore: avoid_print
        print(
          '[B01·category] legacy=${legacyB.b01.category}  new=${newB.b01.category}',
        );
        for (final key in [
          'title',
          'age_range',
  ...
  group('[j7·B01] runJeontongCategory("B01", ctx) 경로 자체도 정상 동작 확인', () {
    for (final u in _seedUsers) {
      test(
        '${u.userId}: JeontongCalcContext + runJeontongCategory("B01") 예외 없이 동작',
        () {
          final newB = _runNew(u, _kFixedDate);
          final rules = SajuFortuneRules.cachedOrNull;
          expect(
          ...
          expect(
            () => result = runJeontongCategory('B01', ctx),
            returnsNormally,
          );
