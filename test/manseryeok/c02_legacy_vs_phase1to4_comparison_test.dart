// [j7 · C02 신규 엔진 이전] 레거시(SajuEngine.calculate) vs 신규
// (PHASE1~4 → sajuResultFromProfile 어댑터) C02(올해 재물운) 결과 비교 테스트.
//
...
//   4) 십신(7키)
//   5) 대운(간지/시작연령/시작연도)
//   6) 신강신약(dayMasterStrength) — C02 자체는 사용하지 않지만 회귀 확인용
//   7) 용신/희신/기신/구신 (신규 전용 — 참고용 로그)
//   8) C02 판단에 실제 쓰이는 계산값(getYearFortune 결과 전체 필드 —
//      yearGanZhi/yearTheme/ganGod/zhiGod/title/overall/wealth/career/
//      love/health/advice)
//   9) 최종 C02 결과(JeontongCategoryResult 전체 필드)
//  10) 결과 문구 전체
//
// [C02의 계산 의존성 — B01과 동일 패턴] C02(`_yearFortuneToResult()` →
...
// C02 결과는 seed-user-A/B/C 전원 완전 일치해야 한다.
...
/// 골든 테스트와 동일한 기준일(kFixedDate) — C02의 `ctx.year`는 이
...
class _LegacyBundle {
  _LegacyBundle(this.saju, this.interp, this.c02);
  final legacy.SajuResult saju;
  final SajuFullInterpretation interp;
  final JeontongCategoryResult c02;
}

class _NewBundle {
  _NewBundle(this.profile, this.saju, this.interp, this.c02);
...
JeontongCategoryResult _runC02(
  ...
  return runJeontongCategory('C02', ctx);
}
...
_LegacyBundle _runLegacy(_SeedUser u, DateTime referenceDate) {
  ...
  final c02 = _runC02(saju, interp, referenceDate);
  return _LegacyBundle(saju, interp, c02);
}
...
_NewBundle _runNew(_SeedUser u, DateTime referenceDate) {
  ...
  final interp = SajuInterpreter.fullInterpretation(saju);
  final c02 = _runC02(saju, interp, referenceDate);
  return _NewBundle(p4, saju, interp, c02);
}
...
void main() {
  ...
  group('[j7·C02] 레거시 vs 신규(PHASE1~4+어댑터) 전체 비교 — seed 유저 3명', () {
    for (final u in _seedUsers) {
      test('${u.userId}: 8글자/일간/오행/십신/대운/신강신약/용희기구/C02 전체 비교', () {
        ...
        for (final key in [
          ...
          final lv = legacyB.c02.data[key];
          final nv = newB.c02.data[key];
          // ignore: avoid_print
          print('[C02·$key] legacy=$lv');
          // ignore: avoid_print
          print('[C02·$key] new   =$nv');
        }
        ...
        // 동일해야 한다 — 이것이 C02의 정상 기대값이다.
        expect(
          newB.c02.data,
          legacyB.c02.data,
          reason:
              '${u.userId}: C02은 dayMasterStrength를 사용하지 않는 카테고리이므로 '
