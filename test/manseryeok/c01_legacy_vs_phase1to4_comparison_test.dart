...
void main() {
  ...
  group('[j7·C01] 레거시 vs 신규(PHASE1~4+어댑터) 전체 비교 — seed 유저 3명', () {
    for (final u in _seedUsers) {
      test('${u.userId}: 8글자/일간/오행/십신/대운/신강신약/용희기구/C01 전체 비교', () {
        ...
        // ignore: avoid_print
        print(
          '[신강신약] new   =${newB.saju.dayMasterStrength}  (${strengthSame ? "동일" : "★ 다름(A01/B01과 동일한 seed-user-C 패턴 - C01엔 영향 없음) ★"})',
        );
        ...
        // ignore: avoid_print
        print(
          '[C01·category] legacy=${legacyB.c01.category}  new=${newB.c01.category}',
        );

        final c01Same = legacyB.c01.data.toString() == newB.c01.data.toString();
        // ignore: avoid_print
        print(
          '[결론] C01 전체 데이터 동일여부=$c01Same '
  ...
  group('[j7·C01] runJeontongCategory("C01", ctx) 경로 자체도 정상 동작 확인', () {
    for (final u in _seedUsers) {
      test(
        '${u.userId}: JeontongCalcContext + runJeontongCategory("C01") 예외 없이 동작',
        () {
          final newB = _runNew(u, _kFixedDate);
          final rules = SajuFortuneRules.cachedOrNull;
          expect(
          ...
          expect(
            () => result = runJeontongCategory('C01', ctx),
            returnsNormally,
          );
