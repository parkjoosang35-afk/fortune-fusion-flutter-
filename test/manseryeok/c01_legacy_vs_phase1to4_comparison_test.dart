// [j7 · C01 신규 엔진 이전] 레거시(SajuEngine.calculate) vs 신규
// (PHASE1~4 → sajuResultFromProfile 어댑터) C01(올해 총운) 결과 비교 테스트.
//
...
//   4) 십신(7키)
//   5) 대운(간지/시작연령/시작연도)
//   6) 신강신약(dayMasterStrength) — C01 자체는 사용하지 않지만 회귀 확인용
//   7) 용신/희신/기신/구신 (신규 전용 — 참고용 로그)
//   8) C01 판단에 실제 쓰이는 계산값(getYearFortune 결과 전체 필드 —
//      yearGanZhi/yearTheme/ganGod/zhiGod/title/overall/wealth/career/
//      love/health/advice)
//   9) 최종 C01 결과(JeontongCategoryResult 전체 필드)
//  10) 결과 문구 전체
//
// [C01의 계산 의존성 — B01과 동일 패턴] C01(`_yearFortuneToResult()` →
...
// C01 결과는 seed-user-A/B/C 전원 완전 일치해야 한다.
...
/// 골든 테스트와 동일한 기준일(kFixedDate) — C01의 `ctx.year`는 이
...
class _LegacyBundle {
  _LegacyBundle(this.saju, this.interp, this.c01);
  final legacy.SajuResult saju;
  final SajuFullInterpretation interp;
  final JeontongCategoryResult c01;
}

class _NewBundle {
  _NewBundle(this.profile, this.saju, this.interp, this.c01);
...
JeontongCategoryResult _runC01(
  ...
  return runJeontongCategory('C01', ctx);
}
...
_LegacyBundle _runLegacy(_SeedUser u, DateTime referenceDate) {
  ...
  final c01 = _runC01(saju, interp, referenceDate);
  return _LegacyBundle(saju, interp, c01);
}
...
_NewBundle _runNew(_SeedUser u, DateTime referenceDate) {
  ...
  final interp = SajuInterpreter.fullInterpretation(saju);
  final c01 = _runC01(saju, interp, referenceDate);
  return _NewBundle(p4, saju, interp, c01);
}
...
void main() {
  ...
  group('[j7·C01] 레거시 vs 신규(PHASE1~4+어댑터) 전체 비교 — seed 유저 3명', () {
    for (final u in _seedUsers) {
      test('${u.userId}: 8글자/일간/오행/십신/대운/신강신약/용희기구/C01 전체 비교', () {
        ...
        // ── 5) 대운(회귀 확인용, C01은 대운 자체는 사용하지 않음) ──
        ...
        // ── 6) 신강신약(dayMasterStrength) — C01엔 미사용, 회귀 확인용 ──
        ...
        // ignore: avoid_print
        print('[신강신약] new   =${newB.saju.dayMasterStrength}  (${strengthSame ? "동일" : "★ 다름(A01/B01과 동일한 seed-user-C 패턴 - C01엔 영향 없음) ★"})');
        ...
        // ── 7) 용신/희신/기신/구신 (신규 전용, C01엔 미사용 — 참고 로그) ──
        ...
        // ── 8) C01 판단에 실제 사용되는 계산값(JeontongCategoryResult 필드) ──
        for (final key in [
          'year_gan_zhi',
          'year_theme',
          'gan_god',
          ...
          final lv = legacyB.c01.data[key];
          final nv = newB.c01.data[key];
          // ignore: avoid_print
          print('[C01·$key] legacy=$lv');
          // ignore: avoid_print
          print('[C01·$key] new   =$nv');
        }

        // ── 9)/10) 최종 C01 결과 전체 동일성 ──
        // ignore: avoid_print
        print('[C01·category] legacy=${legacyB.c01.category}  new=${newB.c01.category}');

        final c01Same = legacyB.c01.data.toString() == newB.c01.data.toString();
        // ignore: avoid_print
        print('[결론] C01 전체 데이터 동일여부=$c01Same '
            '(dayMasterStrength동일=$strengthSame — C01은 신강신약을 사용하지 않으므로 '
            'strength 차이와 무관하게 C01 결과는 일간 계산이 일치하는 한 항상 동일해야 함)');

        // ── C01은 dayMasterStrength에 의존하지 않으므로, 일간이 이미
        // 완전 일치함을 확인했다면(위 2번) legacy/신규 C01 결과는 100%
        // 동일해야 한다 — 이것이 C01의 정상 기대값이다.
        expect(
          newB.c01.data,
          legacyB.c01.data,
          reason:
              '${u.userId}: C01은 dayMasterStrength를 사용하지 않는 카테고리이므로 '
  ...
  group('[j7·C01] runJeontongCategory("C01", ctx) 경로 자체도 정상 동작 확인', () {
    for (final u in _seedUsers) {
      test('${u.userId}: JeontongCalcContext + runJeontongCategory("C01") 예외 없이 동작', () {
        final newB = _runNew(u, _kFixedDate);
        final rules = SajuFortuneRules.cachedOrNull;
        ...
        expect(() => result = runJeontongCategory('C01', ctx), returnsNormally);
        expect(result.category, '${_kFixedDate.year}년 운세');
        expect(result.data['title'], newB.c01.data['title']);
        expect(result.data['overall'], newB.c01.data['overall']);
      });
    }
  });
}
