...
void main() {
  ...
  group('[j7·D01] 레거시 vs 신규(PHASE1~4+어댑터) 전체 비교 — seed 유저 3명', () {
    for (final u in _seedUsers) {
      test('${u.userId}: 8글자/일간/오행/십신/대운/신강신약/용희기구/D01 전체 비교', () {
        ...
        // ignore: avoid_print
        print(
          '[신강신약] new   =${newB.saju.dayMasterStrength}  (${strengthSame ? "동일" : "★ 다름(A01/B01/C01과 동일한 seed-user-C 패턴 - D01엔 영향 없음) ★"})',
        );
        ...
        // ignore: avoid_print
        print(
          '[D01·category] legacy=${legacyB.d01.category}  new=${newB.d01.category}',
        );

        final d01Same = legacyB.d01.data.toString() == newB.d01.data.toString();
        // ignore: avoid_print
        print(
          '[결론] D01 전체 데이터 동일여부=$d01Same '
  ...
  group('[j7·D01] runJeontongCategory("D01", ctx) 경로 자체도 정상 동작 확인', () {
    for (final u in _seedUsers) {
      test(
        '${u.userId}: JeontongCalcContext + runJeontongCategory("D01") 예외 없이 동작',
        () {
          final newB = _runNew(u, _kFixedDate);
          final rules = SajuFortuneRules.cachedOrNull;
          expect(
          ...
          expect(
            () => result = runJeontongCategory('D01', ctx),
            returnsNormally,
          );
