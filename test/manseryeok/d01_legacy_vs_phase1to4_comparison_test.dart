// [j7 · D01 신규 엔진 이전] 레거시(SajuEngine.calculate) vs 신규
// (PHASE1~4 → sajuResultFromProfile 어댑터) D01(이달의 운세) 결과 비교 테스트.
//
// A01/B01/C01과 동일한 원칙(§2/§3) — 단순 "테스트 통과 여부"가 아니라 다음
// 전부를 나란히 출력/비교한다:
//   1) 사주 8글자(년/월/일/시주)
//   2) 일간
//   3) 오행(개수)
//   4) 십신(7키)
//   5) 대운(간지/시작연령/시작연도)
//   6) 신강신약(dayMasterStrength) — D01 자체는 사용하지 않지만 회귀 확인용
//   7) 용신/희신/기신/구신 (신규 전용 — 참고용 로그)
//   8) D01 판단에 실제 쓰이는 계산값(getMonthlyFortune 결과 전체 필드 —
//      monthGanZhi/ganGod/zhiGod/title/overall/work/wealth/love/health/advice)
//   9) 최종 D01 결과(JeontongCategoryResult 전체 필드)
//  10) 결과 문구 전체
//
// [D01의 계산 의존성 — B01/C01과 동일 패턴] D01(`_monthlyFortuneToResult()` →
// `getMonthlyFortune(ctx.saju, ctx.rules, year: ctx.year, month: ctx.month)`)
// 은 월운(이번 달) 간지를 `Solar.fromYmd(year, month, 15)`로 원국과 무관하게
// 직접 구한 뒤, 그 월운 간지와 `saju.dayMaster.gan`(일간)의 십신 관계만으로
// rules JSON을 룩업한다. dayMasterStrength(신강신약)에는 **의존하지 않는다**.
// 따라서 일간이 legacy/신규 완전 일치하는 한(모든 seed 유저에서 이미
// 확인됨), D01 결과는 seed-user-A/B/C 전원 완전 일치해야 한다.
...
/// 골든 테스트와 동일한 기준일(kFixedDate) — D01의 `ctx.year`/`ctx.month`는
/// 이 기준일의 연/월(2026/8)을 그대로 사용한다.
...
class _LegacyBundle {
  _LegacyBundle(this.saju, this.interp, this.d01);
  final legacy.SajuResult saju;
  final SajuFullInterpretation interp;
  final JeontongCategoryResult d01;
}

class _NewBundle {
  _NewBundle(this.profile, this.saju, this.interp, this.d01);
...
JeontongCategoryResult _runD01(
  ...
  return runJeontongCategory('D01', ctx);
}
...
_LegacyBundle _runLegacy(_SeedUser u, DateTime referenceDate) {
  ...
  final d01 = _runD01(saju, interp, referenceDate);
  return _LegacyBundle(saju, interp, d01);
}
...
_NewBundle _runNew(_SeedUser u, DateTime referenceDate) {
  ...
  final interp = SajuInterpreter.fullInterpretation(saju);
  final d01 = _runD01(saju, interp, referenceDate);
  return _NewBundle(p4, saju, interp, d01);
}
...
void main() {
  ...
  group('[j7·D01] 레거시 vs 신규(PHASE1~4+어댑터) 전체 비교 — seed 유저 3명', () {
    for (final u in _seedUsers) {
      test('${u.userId}: 8글자/일간/오행/십신/대운/신강신약/용희기구/D01 전체 비교', () {
        ...
        // ── 5) 대운(회귀 확인용, D01은 대운 자체는 사용하지 않음) ──
        ...
        // ── 6) 신강신약(dayMasterStrength) — D01엔 미사용, 회귀 확인용 ──
        ...
        // ignore: avoid_print
        print('[신강신약] new   =${newB.saju.dayMasterStrength}  (${strengthSame ? "동일" : "★ 다름(A01/B01/C01과 동일한 seed-user-C 패턴 - D01엔 영향 없음) ★"})');
        ...
        // ── 7) 용신/희신/기신/구신 (신규 전용, D01엔 미사용 — 참고 로그) ──
        ...
        // ── 8) D01 판단에 실제 사용되는 계산값(JeontongCategoryResult 필드) ──
        for (final key in [
          'month_gan_zhi',
          'gan_god',
          'zhi_god',
          ...
          final lv = legacyB.d01.data[key];
          final nv = newB.d01.data[key];
          // ignore: avoid_print
          print('[D01·$key] legacy=$lv');
          // ignore: avoid_print
          print('[D01·$key] new   =$nv');
        }

        // ── 9)/10) 최종 D01 결과 전체 동일성 ──
        // ignore: avoid_print
        print('[D01·category] legacy=${legacyB.d01.category}  new=${newB.d01.category}');

        final d01Same = legacyB.d01.data.toString() == newB.d01.data.toString();
        // ignore: avoid_print
        print('[결론] D01 전체 데이터 동일여부=$d01Same '
            '(dayMasterStrength동일=$strengthSame — D01은 신강신약을 사용하지 않으므로 '
            'strength 차이와 무관하게 D01 결과는 일간 계산이 일치하는 한 항상 동일해야 함)');

        // ── D01은 dayMasterStrength에 의존하지 않으므로, 일간이 이미
        // 완전 일치함을 확인했다면(위 2번) legacy/신규 D01 결과는 100%
        // 동일해야 한다 — 이것이 D01의 정상 기대값이다.
        expect(
          newB.d01.data,
          legacyB.d01.data,
          reason:
              '${u.userId}: D01은 dayMasterStrength를 사용하지 않는 카테고리이므로 '
  ...
  group('[j7·D01] runJeontongCategory("D01", ctx) 경로 자체도 정상 동작 확인', () {
    for (final u in _seedUsers) {
      test('${u.userId}: JeontongCalcContext + runJeontongCategory("D01") 예외 없이 동작', () {
        final newB = _runNew(u, _kFixedDate);
        final rules = SajuFortuneRules.cachedOrNull;
        ...
        expect(() => result = runJeontongCategory('D01', ctx), returnsNormally);
        expect(result.data['title'], newB.d01.data['title']);
        expect(result.data['overall'], newB.d01.data['overall']);
      });
    }
  });
}
