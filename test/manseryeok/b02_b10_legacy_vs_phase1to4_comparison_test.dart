...
void main() {
  ...
  group('[j9·B02~B10] 카테고리 등록/제외 상태 확인', () {
    test('kJeontongPlaceholderCategoryIds에 B02~B10이 더 이상 없어야 함', () {
      for (final id in _bIds) {
        expect(kJeontongPlaceholderCategoryIds.contains(id), isFalse, reason: '$id는 실계산으로 전환되어 플레이스홀더 목록에서 빠져야 함');
      }
    });
  });
  ...
  group('[j9·B02~B10] 레거시 vs 신규(PHASE1~4+어댑터) 대운 목록 비교(공통 접두사) — seed 유저 3명', () {
    for (final u in _seedUsers) {
      test('${u.userId}: 대운 간지 목록의 공통 접두사(legacy 8개)는 legacy/new 동일, new는 9번째를 추가로 보유', () {
        final legacyB = _runLegacy(u, _kFixedDate);
        final newB = _runNew(u, _kFixedDate);

        final legacyReal = legacyB.saju.luckPillars.where((lp) => lp.ganZhiKr.isNotEmpty).toList();
        final newReal = newB.saju.luckPillars.where((lp) => lp.ganZhiKr.isNotEmpty).toList();
        ...
        expect(legacyReal.length, 8, reason: 'legacy는 소운기 슬롯 낭비로 실제 대운이 항상 8개여야 함(회귀 확인)');
        expect(newReal.length, 9, reason: 'new는 요청한 count(9)만큼 실제 대운을 항상 채워야 함(회귀 확인)');
        expect(
          newReal.take(8).map((e) => e.ganZhiKr).toList(),
          legacyReal.map((e) => e.ganZhiKr).toList(),
          reason: '${u.userId}: 공통 접두사(첫 8개)는 8글자가 동일하므로 legacy/new가 완전히 동일해야 함',
        );
      });
    }
  });
  ...
  group('[j9·B02~B07,B10] legacy/new 결과 비교(공통 접두사 기준, 용신 불필요 항목)', () {
    for (final u in _seedUsers) {
      test('${u.userId}: B02/B03/B04/B05/B06/B07/B10 — legacy 타임라인은 new 타임라인의 접두사와 동일', () {
        final legacyB = _runLegacy(u, _kFixedDate);
        final newB = _runNew(u, _kFixedDate);
        ...
        for (final id in ['B02', 'B03', 'B04', 'B05', 'B06', 'B07']) {
          ...
          if (legacyTimeline != null && newTimeline != null) {
            expect(
              ...
              legacyTimeline,
              reason: '${u.userId}: $id의 timeline은 공통 접두사(legacy 길이만큼)가 legacy/new 완전히 동일해야 함',
            );
          }
        }
        ...
        expect(
          ...
          reason: '${u.userId}: B10은 currentLuck 1건 기준이므로 legacy/new가 완전히 동일해야 함',
        );
      });
    }
  });
  ...
  group('[j9·B08/B09] 용신/기신 기반 최고·최악 대운 — profile 유무에 따른 동작 확인', () {
    for (final u in _seedUsers) {
      test('${u.userId}: legacy(profile=null)는 판단 불가, new(profile 有)는 실제 판정', () {
        ...
        expect(legacyB.results['B08']!.data['summary'], contains('판단하기 어려워요'));
        expect(legacyB.results['B09']!.data['periods'], isEmpty);
        expect(legacyB.results['B09']!.data['summary'], contains('판단하기 어려워요'));
        ...
        // ignore: avoid_print
        print('[${u.userId}] yongsin=${yongsin?.yongsin} gisin=${yongsin?.gisin}');
        // ignore: avoid_print
        print('[B08] new=${newB.results['B08']!.data}');
        // ignore: avoid_print
        print('[B09] new=${newB.results['B09']!.data}');
        expect(newB.results['B08']!.data['summary'], isNot(contains('판단하기 어려워요')));
        expect(newB.results['B09']!.data['summary'], isNot(contains('판단하기 어려워요')));
      });
    }
  });
  ...
  group('[j9·B02~B10] runJeontongCategory 경로 자체도 정상 동작 확인', () {
    for (final u in _seedUsers) {
      test('${u.userId}: JeontongCalcContext(profile 포함) + runJeontongCategory("B02"~"B10") 예외 없이 동작', () {
        final newB = _runNew(u, _kFixedDate);
        final rules = SajuFortuneRules.cachedOrNull;
