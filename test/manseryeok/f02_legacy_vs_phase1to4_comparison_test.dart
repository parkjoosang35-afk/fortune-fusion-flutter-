...
void main() {
  ...
  group('[j7·F02] 레거시 vs 신규(PHASE1~4+어댑터) 전체 비교 — seed 유저 3명', () {
    for (final u in _seedUsers) {
      test('${u.userId}: 8글자/일간/오행/십신/대운/신강신약/용희기구/F02 전체 비교', () {
        ...
        // ignore: avoid_print
        print('[신강신약] new   =${newB.saju.dayMasterStrength}  (${strengthSame ? "동일" : "★ 다름 — F02 work_style 필드에만 영향 ★"})');
        ...
        // ignore: avoid_print
        print('[F02계산값] legacy officer=$legacyOfficer printer=$legacyPrinter output=$legacyOutput');
        // ignore: avoid_print
        print('[F02계산값] new    officer=$newOfficer printer=$newPrinter output=$newOutput');
        ...
        // ignore: avoid_print
        print('[F02계산값] legacy dayMasterAnalysis.title=${legacyB.interp.dayMasterAnalysis.title}');
        // ignore: avoid_print
        print('[F02계산값] new    dayMasterAnalysis.title=${newB.interp.dayMasterAnalysis.title}');
        ...
        // ── 10) 최종 F02 결과 전체 동일성 ──
        // ignore: avoid_print
        print('[F02·category] legacy=${legacyB.f02.category}  new=${newB.f02.category}');
        expect(legacyB.f02.category, '평생 직업·명예운');
        expect(newB.f02.category, '평생 직업·명예운');

        final structureSame = legacyB.f02.data['structure'] == newB.f02.data['structure'];
        final workStyleSame = legacyB.f02.data['work_style'] == newB.f02.data['work_style'];
        ...
        // 검증이다.
        expect(newB.f02.data['structure'], legacyB.f02.data['structure'],
            reason: '${u.userId}: structure는 dayMasterStrength와 무관하므로 항상 동일해야 함');
        expect(newB.f02.data['message'], legacyB.f02.data['message'],
            reason: '${u.userId}: message는 dayMasterStrength와 무관하므로 항상 동일해야 함');
        expect(newB.f02.data['recommended_jobs'], legacyB.f02.data['recommended_jobs'],
            reason: '${u.userId}: recommended_jobs는 dayMasterStrength와 무관하므로 항상 동일해야 함');
        expect(newB.f02.data['growth_path'], legacyB.f02.data['growth_path'],
  ...
  group('[j7·F02] runJeontongCategory("F02", ctx) 경로 자체도 정상 동작 확인', () {
    for (final u in _seedUsers) {
      test('${u.userId}: JeontongCalcContext + runJeontongCategory("F02") 예외 없이 동작', () {
        final newB = _runNew(u, _kFixedDate);
        final rules = SajuFortuneRules.cachedOrNull;
        ...
        expect(() => result = runJeontongCategory('F02', ctx), returnsNormally);
