...
void main() {
  ...
  group('[j7·C05] 레거시 vs 신규(PHASE1~4+어댑터) 전체 비교 — seed 유저 3명', () {
    for (final u in _seedUsers) {
      test('${u.userId}: 8글자/일간/오행/십신/대운/신강신약/용희기구/C05 전체 비교', () {
        ...
        // ignore: avoid_print
        print('[신강신약] new   =${newB.saju.dayMasterStrength}  (${strengthSame ? "동일" : "★ 다름(A01/B01과 동일한 seed-user-C 패턴 - C05엔 영향 없음) ★"})');
        ...
        // ignore: avoid_print
        print('[C05·category] legacy=${legacyB.c05.category}  new=${newB.c05.category}');

        final c05Same = legacyB.c05.data.toString() == newB.c05.data.toString();
        // ignore: avoid_print
        print('[결론] C05 전체 데이터 동일여부=$c05Same '
  ...
  group('[j7·C05] runJeontongCategory("C05", ctx) 경로 자체도 정상 동작 확인', () {
    for (final u in _seedUsers) {
      test('${u.userId}: JeontongCalcContext + runJeontongCategory("C05") 예외 없이 동작', () {
        final newB = _runNew(u, _kFixedDate);
        final rules = SajuFortuneRules.cachedOrNull;
        ...
        expect(() => result = runJeontongCategory('C05', ctx), returnsNormally);
