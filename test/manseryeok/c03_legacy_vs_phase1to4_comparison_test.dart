...
void main() {
  ...
  group('[j7·C03] 레거시 vs 신규(PHASE1~4+어댑터) 전체 비교 — seed 유저 3명', () {
    for (final u in _seedUsers) {
      test('${u.userId}: 8글자/일간/오행/십신/대운/신강신약/용희기구/C03 전체 비교', () {
        ...
        // ignore: avoid_print
        print(
          '[신강신약] new   =${newB.saju.dayMasterStrength}  (${strengthSame ? "동일" : "★ 다름(A01/B01과 동일한 seed-user-C 패턴 - C03엔 영향 없음) ★"})',
        );
        ...
        // ignore: avoid_print
        print(
          '[C03·category] legacy=${legacyB.c03.category}  new=${newB.c03.category}',
        );

        final c03Same = legacyB.c03.data.toString() == newB.c03.data.toString();
        // ignore: avoid_print
        print(
          '[결론] C03 전체 데이터 동일여부=$c03Same '
  ...
  group('[j7·C03] runJeontongCategory("C03", ctx) 경로 자체도 정상 동작 확인', () {
    for (final u in _seedUsers) {
      test(
        '${u.userId}: JeontongCalcContext + runJeontongCategory("C03") 예외 없이 동작',
        () {
          final newB = _runNew(u, _kFixedDate);
          final rules = SajuFortuneRules.cachedOrNull;
          expect(
          ...
          expect(
            () => result = runJeontongCategory('C03', ctx),
            returnsNormally,
          );
