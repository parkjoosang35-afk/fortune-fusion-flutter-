// [j7 · A03 신규 엔진 이전] 레거시(SajuEngine.calculate) vs 신규
// (PHASE1~4 → sajuResultFromProfile 어댑터) A03(평생 재물운) 결과 비교 테스트.
//
...
//   4) 십신(7키)
//   5) 대운(간지/시작연령/시작연도)
//   6) 신강신약(dayMasterStrength) — A03의 verdict 분기에 직접 사용됨
//   7) 용신/희신/기신/구신 (신규 전용 — 참고용 로그)
//   8) A03 판단에 실제 쓰이는 계산값(wealthCount, dayMasterStrength)
//   9) 최종 A03 결과(verdict/structure/message/asset_style/peak_period)
//  10) 결과 문구 전체
//
// [A03의 계산 의존성 — A01/A02보다 위험도 높음]
// `_a03(ctx)`(jeontong_eighty_calculator.dart) → `getLifeWealth(interp)`
...
// 신강/중화/신약 라벨)만 바뀌고 핵심 문구 구조는 유지되었지만, A03은
...
class _LegacyBundle {
  _LegacyBundle(this.saju, this.interp, this.a03);
  final legacy.SajuResult saju;
  final SajuFullInterpretation interp;
  final JeontongCategoryResult a03;
}

class _NewBundle {
  _NewBundle(this.profile, this.saju, this.interp, this.a03);
...
JeontongCategoryResult _runA03(
  ...
  return runJeontongCategory('A03', ctx);
}
...
_LegacyBundle _runLegacy(_SeedUser u, DateTime referenceDate) {
  ...
  final a03 = _runA03(saju, interp, referenceDate);
  return _LegacyBundle(saju, interp, a03);
}
...
_NewBundle _runNew(_SeedUser u, DateTime referenceDate) {
  ...
  final interp = SajuInterpreter.fullInterpretation(saju);
  final a03 = _runA03(saju, interp, referenceDate);
  return _NewBundle(p4, saju, interp, a03);
}
...
void main() {
  ...
  group('[j7·A03] 레거시 vs 신규(PHASE1~4+어댑터) 전체 비교 — seed 유저 3명', () {
    for (final u in _seedUsers) {
      test('${u.userId}: 8글자/일간/오행/십신/대운/신강신약/용희기구/A03 전체 비교', () {
        ...
        // ── 5) 대운(회귀 확인용, A03은 대운 자체는 사용하지 않음) ──
        ...
        // ── 6) 신강신약(dayMasterStrength) — A03 verdict 분기의 핵심 입력값 ──
        ...
        // ignore: avoid_print
        print('[신강신약] new   =${newB.saju.dayMasterStrength}  (${strengthSame ? "동일" : "★ 다름 — A03 verdict 자체가 바뀔 수 있음 ★"})');
        ...
        // ── 7) 용신/희신/기신/구신 (신규 전용, A03엔 미사용 — 참고 로그) ──
        ...
        // ── 8) A03 판단에 실제 사용되는 계산값(wealthCount, dominant 십신) ──
        ...
        // ignore: avoid_print
        print('[A03계산값] legacy wealthGodCount=$legacyWealthCount (재성 개수, verdict 1차 분기)');
        // ignore: avoid_print
        print('[A03계산값] new    wealthGodCount=$newWealthCount');
        expect(newWealthCount, legacyWealthCount, reason: '${u.userId} 재성(정재+편재) 개수는 십신이 일치하므로 동일해야 함');
        // ignore: avoid_print
        print('[A03계산값] legacy verdict=${legacyB.interp.wealthFortune.verdict}');
        // ignore: avoid_print
        print('[A03계산값] new    verdict=${newB.interp.wealthFortune.verdict}');

        // ── 9) A03 판단에 실제 사용되는 계산값(JeontongCategoryResult 필드) ──
        for (final key in [
          'verdict',
          'structure',
          'message',
          ...
          final lv = legacyB.a03.data[key];
          final nv = newB.a03.data[key];
          // ignore: avoid_print
          print('[A03·$key] legacy=$lv');
          // ignore: avoid_print
          print('[A03·$key] new   =$nv');
        }

        // ── 10) 최종 A03 결과 전체 동일성 ──
        // ignore: avoid_print
        print('[A03·category] legacy=${legacyB.a03.category}  new=${newB.a03.category}');
        expect(legacyB.a03.category, '평생 재물운');
        expect(newB.a03.category, '평생 재물운');

        final verdictSame = legacyB.a03.data['verdict'] == newB.a03.data['verdict'];
        final a03Same = legacyB.a03.data.toString() == newB.a03.data.toString();
        // ignore: avoid_print
        print('[결론] verdict동일=$verdictSame A03 전체 데이터 동일여부=$a03Same '
            '(dayMasterStrength동일=$strengthSame) → '
            '${!strengthSame && !verdictSame ? "dayMasterStrength 차이가 A03 verdict 자체를 바꿈(재물운 유형 재분류 — PHASE3 기준 정상 결과)" : (strengthSame && a03Same ? "완전 일치" : (verdictSame && !a03Same ? "verdict는 동일하나 세부 문구 차이 있음 - 확인 필요" : "다른 원인으로 차이 발생 - 확인 필요"))}');

        // ── 구조적 안전성만 강제(문구 1:1 동일성은 요구하지 않음 — §2/§3
        // 원칙). A03은 dayMasterStrength를 verdict 분기 조건 자체로 사용
        // 하므로, dayMasterStrength가 다른 경우(seed-user-C 패턴) verdict
        // 문자열 자체가 달라지는 것도 정상적인 엔진 개선일 수 있다.
        expect(newB.a03.data['verdict'], isNotEmpty);
        expect(newB.a03.data['message'], isNotEmpty);
        expect(newB.a03.data['structure'], isNotEmpty);
        expect(newB.a03.data['asset_style'], isNotEmpty);
        expect(newB.a03.data['peak_period'], isNotEmpty);
        // verdict는 항상 5가지 정의된 값 중 하나여야 한다(getLifeWealth 공식).
        expect(
          newB.a03.data['verdict'],
        ...
        expect(
          newB.a03.data['asset_style'],
          assetStyleByVerdict[newB.a03.data['verdict']],
          reason: '${u.userId}: asset_style은 verdict에 정확히 대응해야 함(폴백 사용 금지)',
        );

        // dayMasterStrength가 legacy와 동일한 seed라면 A03 결과도
        ...
        if (strengthSame) {
          expect(
            ...
            legacyB.a03.data,
            reason:
                '${u.userId}: dayMasterStrength가 legacy와 동일하므로 A03 결과도 '
                '완전히 동일해야 합니다.',
          );
        }
      });
  ...
  group('[j7·A03] runJeontongCategory("A03", ctx) 경로 자체도 정상 동작 확인', () {
    for (final u in _seedUsers) {
      test('${u.userId}: JeontongCalcContext + runJeontongCategory("A03") 예외 없이 동작', () {
        final newB = _runNew(u, _kFixedDate);
        final rules = SajuFortuneRules.cachedOrNull;
        ...
        expect(() => result = runJeontongCategory('A03', ctx), returnsNormally);
        expect(result.category, '평생 재물운');
        expect(result.data['verdict'], newB.a03.data['verdict']);
        expect(result.data['message'], newB.a03.data['message']);
      });
    }
  });
}
